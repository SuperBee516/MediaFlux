...
internal sealed class VideoRestorationSettingsForm : MediaFluxForm
{
    ...
    public VideoRestorationSettingsForm(VideoRestorationSettings settings)
    {
        ...
        Set(_aiMode, _working.AiMode); Set(_aiScale, _working.AiScale); _persistedModelId = _working.AiModelId; _aiDevice.Text = string.IsNullOrWhiteSpace(_working.AiDevice) ? "Auto" : _working.AiDevice; _aiBackend.Text = _working.AiBackendPath; _aiModels.Text = _working.AiModelsDirectory;
    ...
    private async Task RefreshModelsAsync()
    {
        ...
        var probe = _working.Clone(); probe.AiMode = mode; probe.AiScale = Get<AiRestorationScale>(_aiScale); probe.AiDevice = string.IsNullOrWhiteSpace(_aiDevice.Text) ? "Auto" : _aiDevice.Text.Trim(); probe.AiBackendPath = _aiBackend.Text.Trim(); probe.AiModelsDirectory = _aiModels.Text.Trim();
        try
        {
            AiRestorationCapabilities capabilities = await new Services.AiRestorationBackendService(AppPaths.InstallDirectory).GetCapabilitiesAsync(probe);
            ...
            _aiModelStatus.Text = choices.Length > 0 ? $"{choices.Length} compatible local model pair(s) detected for {(int)probe.AiScale}x." : capabilities.Error ?? $"No complete compatible local model pair was found for {(int)probe.AiScale}x.";
        }
