...
public sealed record AiIntermediateVideoRequest(string SourcePath, double FrameRate, TimeSpan SourceDuration, VideoRestorationSettings Settings, VideoRestorationPipelinePlan Plan, TimeSpan? Start = null, TimeSpan? Duration = null);
public sealed record AiIntermediateVideoResult(string Path, TimeSpan Duration, int FrameCount, int Width, int Height, double FrameRate) : IDisposable
...
public sealed class AiRestorationIntermediateVideoService
{
    ...
    public async Task<AiIntermediateVideoResult> CreateAsync(AiIntermediateVideoRequest request, IProgress<AiIntermediateProgress>? progress = null, CancellationToken token = default)
    {
        ...
        await _backend.ValidateSelectionAsync(request.Settings, token).ConfigureAwait(false);
        string root = Path.Combine(_stagingRoot, "ai-intermediate-" + Guid.NewGuid().ToString("N")); Directory.CreateDirectory(root);
        try
        {
            ...
            File.Move(staging, final, true); return result with { Path = final };
        }
