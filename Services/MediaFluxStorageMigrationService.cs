...
public sealed class MediaFluxStorageMigrationService
{
    ...
    public async Task<MediaFluxStorageMigrationResult> MigrateAsync(string destination, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            PromoteVerifiedStaging(staging, target); // The picker may create an empty target; remove only that verified-empty shell before the atomic rename.
    ...
    private static void PromoteVerifiedStaging(string staging, string target)
    {
        if (File.Exists(target)) throw new IOException("The destination path became a file during migration.");
        if (Directory.Exists(target))
        {
            if (Directory.EnumerateFileSystemEntries(target).Any()) throw new IOException("The destination folder received files during migration and was left unchanged.");
