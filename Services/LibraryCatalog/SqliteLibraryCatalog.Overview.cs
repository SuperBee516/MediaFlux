...
public sealed partial class SqliteLibraryCatalog
{
    ...
    public LibraryOverviewSnapshot GetOverviewSnapshot(int metadataVersion)
    {
        ...
        command.CommandText = $"""
            ...
            SELECT {LibraryCatalogSqlExpressions.ResolutionTier("m")},COUNT(*),COALESCE(SUM(f.size_bytes),0) FROM indexed_files f LEFT JOIN media_metadata m ON m.file_id=f.id GROUP BY 1 ORDER BY 3 DESC,1;
