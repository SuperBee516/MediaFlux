...
public sealed class EncodingDiagnosticsService : IDisposable
{
    ...
    private static EncodingDiagnosticSnapshot Snapshot(Session s){EncodingDiagnosticSample? latest=s.Samples.LastOrDefault();TimeSpan elapsed=DateTime.UtcNow-s.Started;TimeSpan? eta=s.Progress.Speed>0&&s.Job.SourceDurationSeconds is>0?TimeSpan.FromSeconds(Math.Max(0,s.Job.SourceDurationSeconds.Value-s.Progress.MediaSeconds)/s.Progress.Speed):null;return new(s.Job,s.Started,elapsed,eta,latest,EncodingDiagnosticInterpreter.Interpret(latest),s.Samples.Count);}
