...
public sealed class EncodingDiagnosticsService : IDisposable
{
    ...
    {value=default;if(string.IsNullOrWhiteSpace(line))return false;double time=ParseTime(Value(line,"time="));long frame=ParseFrame(Value(line,"frame="));double speed=Number(Value(line,"speed=").TrimEnd('x'));double fps=Number(Value(line,"fps="));double bitrate=Number(Value(line,"bitrate=").Replace("kbits/s","",StringComparison.OrdinalIgnoreCase));string sizeText=Value(line,"size=");long? size=ParseSize(sizeText);if(time<0&&frame>0&&authoritativeTotalFrames is>0&&authoritativeFrameRate is>0&&double.IsFinite(authoritativeFrameRate.Value)){time=frame/authoritativeFrameRate.Value;double percent=Math.Clamp(frame/(double)authoritativeTotalFrames.Value*100,0,100);value=new(speed,fps,bitrate,time,percent,size);return true;}if(time<0)return false;double timestampPercent=totalDurationSeconds is>0?Math.Clamp(time/totalDurationSeconds.Value*100,0,100):0;value=new(speed,fps,bitrate,time,timestampPercent,size);return true;}
