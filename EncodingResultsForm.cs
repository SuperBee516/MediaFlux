...
public sealed class EncodingResultsForm : MediaFluxForm
{
    ...
    private void ConfigureCalibrationCohorts()
    {
        ...
        Add(_calibrationCohorts, "policyId", "Adaptive policy version", 210);
    ...
    private void Bind()
    {
        ...
        EncodingCalibrationEffectiveness[] currentPolicyCohorts = calibrationCohorts
            .Where(item => item.PolicyId == AdaptivePredictionPolicies.Current.PolicyId).ToArray();
        double? medianCalibrationImprovement = Median(currentPolicyCohorts.Where(item => item.EvaluationCount >= AdaptivePredictionPolicies.Current.MinimumCalibrationEffectivenessSamples)
            .Select(item => item.MedianImprovementPercent));
        _summary.Text = $"Clean completed: {metrics.CompletedCount} • Size N: {metrics.SizePredictionCount} (median APE {Percent(metrics.MedianSizeAbsolutePercentageError)}) • ETA N: {metrics.EtaPredictionCount} (median APE {Percent(metrics.MedianEtaAbsolutePercentageError)})\r\nPost-cutoff candidates (recognized policy versions): {calibration.CalibratedCount} • {AdaptivePredictionPolicies.Current.PolicyId} cohorts: {currentPolicyCohorts.Count(item => item.State == EncodingCalibrationEffectivenessState.Effective)} effective / {currentPolicyCohorts.Count(item => item.State == EncodingCalibrationEffectivenessState.Mixed)} mixed / {currentPolicyCohorts.Count(item => item.State == EncodingCalibrationEffectivenessState.Harmful)} harmful • Median improvement (N≥{AdaptivePredictionPolicies.Current.MinimumCalibrationEffectivenessSamples}): {Percent(medianCalibrationImprovement)}";
        ...
        _calibrationCohorts.Rows.Clear(); foreach (EncodingCalibrationEffectiveness cohort in calibrationCohorts)
        {
            ...
            int index = _calibrationCohorts.Rows.Add(cohort.CohortKey, cohort.PolicyId, cohort.HistoricalConfidence, cohort.HistoricalSampleCount,
    ...
    private void ShowDetails()
    {
        ...
        double? appliedCorrection = r.CalibrationAppliedCorrectionPercent ??
            (r.CalibrationApplied == true ? r.CalibrationCorrectionPercent : null);
        string calibrationDetail = $"Adaptive policy: historical size-prediction calibration; policy version: {AdaptivePredictionPolicies.NormalizePolicyId(r.CalibrationPolicyId)}; decision: {Value(r.CalibrationDecision, r.CalibrationApplied == true ? "Applied" : "Not available")}; learning strength: {r.CalibrationLearningStrength?.ToString("0.##") ?? "Unavailable"}; raw historical bias: {Percent(r.CalibrationRawHistoricalCorrectionPercent ?? r.CalibrationMedianSignedErrorPercent)}; effective correction: {Percent(appliedCorrection)}; effectiveness state: {Value(r.CalibrationEffectivenessState)}; evidence cutoff={r.CalibrationEvidenceCutoffUtc?.ToLocalTime().ToString("g") ?? "Unavailable"}; evaluation N={r.CalibrationEvaluationCount?.ToString() ?? "0"}. Base {Bytes(r.BasePredictedOutputSizeBytes)}; displayed {Bytes(r.PredictedOutputSizeBytes)}; hypothetical {Bytes(r.HypotheticalCalibratedOutputSizeBytes)}; reason={Value(r.CalibrationReason)}" +
