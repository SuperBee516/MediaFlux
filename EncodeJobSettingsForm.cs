...
internal sealed class EncodeJobSettingsForm : MediaFluxForm
{
    ...
    public EncodeJobSettingsForm(string jobName, EncodeJobSettings settings)
    {
        ...
        Text = $"Encode Settings — {jobName}"; StartPosition = FormStartPosition.CenterParent; ClientSize = new Size(640, 590);
        ...
        var grid = new TableLayoutPanel { Dock = DockStyle.Fill, AutoScroll = true, Padding = new Padding(12), ColumnCount = 2, RowCount = 17 };
        ...
        Add(grid, "Target size (MB)", _target); Add(grid, "Auto quality (CRF/CQ)", _quality); Add(grid, "Resolution", _resolution); Add(grid, "Audio layout", _audio); Add(grid, "Output container", _container);
