...
internal sealed class FfmpegInstallationProgressForm : MediaFluxForm
{
    private readonly Func<IProgress<ManagedComponentProvisioningProgress>, CancellationToken, Task<ManagedComponentProvisioningResult>> _operation;
    ...
    public FfmpegInstallationProgressForm(Func<IProgress<ManagedComponentProvisioningProgress>, CancellationToken, Task<ManagedComponentProvisioningResult>> operation)
    {
        _operation = operation;
        Text = "Install FFmpeg";
        StartPosition = FormStartPosition.CenterParent;
        ClientSize = new Size(500, 165);
        ...
        _stage.Text = "Preparing the compatible FFmpeg package…";
        ...
        Shown += RunAsync;
        FormClosing += (_, e) => { if (!_finished) { Cancel(); e.Cancel = true; } };
    }

    public ManagedComponentProvisioningResult? Result { get; private set; }
    public bool WasCanceled { get; private set; }

    private async void RunAsync(object? sender, EventArgs e)
    {
        try { Result = await _operation(new Progress<ManagedComponentProvisioningProgress>(Update), _cancellation.Token); DialogResult = Result.Succeeded ? DialogResult.OK : DialogResult.Abort; }
        catch (OperationCanceledException) { WasCanceled = true; DialogResult = DialogResult.Cancel; }
        finally { _finished = true; Close(); }
    }

    private void Update(ManagedComponentProvisioningProgress progress)
    {
        _stage.Text = progress.Stage switch
        {
            ManagedComponentProvisioningStage.Downloading => "Downloading FFmpeg…",
            ManagedComponentProvisioningStage.Verifying => "Verifying package…",
            ManagedComponentProvisioningStage.Extracting => "Extracting package…",
            ManagedComponentProvisioningStage.Validating => "Validating FFmpeg and FFprobe…",
            ManagedComponentProvisioningStage.Installing => "Installing FFmpeg…",
        ...
        _detail.Text = progress.TotalBytes is > 0 ? $"{progress.BytesTransferred / 1024d / 1024d:0.0} MB of {progress.TotalBytes.Value / 1024d / 1024d:0.0} MB" : string.Empty;
    }

    private void Cancel()
    {
        if (_finished || _cancellation.IsCancellationRequested) return;
        _cancellation.Cancel(); _cancel.Enabled = false; _stage.Text = "Canceling and cleaning temporary files…"; _progress.Style = ProgressBarStyle.Marquee;
    }
